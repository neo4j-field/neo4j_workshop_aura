...
5. Open the terminal and run the below command for any supported task. Please make sure you have updated the required parameters under the task in `config.json`
   ...
   ```shell
   % python main.py <tenant_id> <client Id> <client secret> <task> --output /path_to_folder/csvfile.csv
   ```
    - Tenant ID (Eg: "6e748720-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
    - Client ID and Client Secret (for Aura API access)
    - Instance ID for the new database (Eg: "44683a64")
...
**If you are running a workshop, you will want readable passwords**

6. Open the terminal and run the below command

   ```shell
    % python auradb_create_password_files.py /path_to_folder/csvfile_readable_pw.csv
   ```
    - input: output filename/path from step 4
    - output: csv with _readable_pw suffix added

Use `auradb_create_password_files.py` after all of the instances are up and running to create and update login information. If you run this before the instances are running, you will get an `Unable to retrieve routing information error`

## Generate student environment files
**If you are running a workshop, you will need an environment file or printed credentials to hand out for each participant.**

Open the terminal and run the appropriate script for your workshop needs.
7. For student environment files, run the below command

   ```shell
    % python generate_handouts.py /path_to_folder/csvfile.csv
   ```
    - input: output filename/path ***prefix*** from step 6 (i.e. instances)
    - output: students folder with aura.txt files 

Upload these files to a public Cloud storage for it to be 

8. For student environment files, run the below command
...
## Generate workshop handouts (OPTIONAL) 
...
Here's a sample execution flow 

```shell
  % python ./main.py <tenant_id> <client Id> <client secret> clone

  % python ./main.py <tenant_id> <client Id> <client secret> status
